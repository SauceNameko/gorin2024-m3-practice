...
export const Event = ({ event, onIcon, onToggle, isCommand, onCountChange, item }) => {
        ...
        <>
            <div className="pos">
                ...
                {spots.map((spot, spotIndex) => {
                    return <>
                        <div className={`icon icon-${spotIndex}`}
                            style={{ top: `${spot.location_y}px`, left: `${spot.location_x}px`, backgroundColor: item.map.icon }} onClick={() => onIcon(event.id, spot.id, "spot_tapped")} ></div>
                    </>
                })}
